...
export default memo(function Navbar({
  ...
  useEffect(() => {
    if (!isLoading) {
      if (showRightNav && sidebarStatus) toggleSidebar(false);
  ...
  return (
    <>
      <span className="bars" onClick={(e) => onClickBars(e)}>
        <FontAwesomeIcon icon={properIcon} />
      </span>
      <div className="dashboard-brand">
        <Logo settings={settings} isDemo={isDemo} />
      </div>
      <span className="dots" onClick={() => toggleRightNav((show) => !show)}>
        <FontAwesomeIcon icon={rightNavProperIcon} />
      </span>

      <nav className="right-nav" ref={rightNav}>
        <div className="d-md-none d-flex flex-column align-items-center mb-4">
          <AvatarPalceholder username={user.username} size={100} /> Hi,{" "}
          {user.username}
        </div>
        <div className="d-md-none">
          <Link to="/account/settings">
            <TablerIcon icon={IconUser} /> Profile
          </Link>
          <Link to="/logout">
            <TablerIcon icon={IconLogout} /> Logout
          </Link>
        </div>
      </nav>
      <div className="btn-group avatar-dropdown">
        <button
          ...
          aria-expanded="false"
        >
          <AvatarPalceholder username={user.username} size={45} /> Hi,{" "}
          {user.username}
        </button>
