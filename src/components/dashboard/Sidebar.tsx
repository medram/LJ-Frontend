...
export default memo(function Sidebar({ show }: SidebarProps) {
  ...
  return (
    <aside className={`${classes} d-flex flex-column justify-content-between`}>
      <nav>
        <NavLink to="" end>
          <TablerIcon icon={IconLayoutDashboard} stroke={1.25} size={30} />{" "}
          Dashboard
        </NavLink>
        ...
        {isLC && (
          <>
            <NavLink to="plans">
              <TablerIcon icon={IconPackages} stroke={1.25} size={30} /> Plans
            </NavLink>
            <NavLink to="subscriptions">
              <FontAwesomeIcon icon={faGem} size="lg" /> Subscriptions
            </NavLink>
            <NavLink to="payment-gateways">
              <TablerIcon icon={IconCreditCard} stroke={1.25} size={30} />{" "}
              Payment Gateways
            </NavLink>
          </>
        )}
