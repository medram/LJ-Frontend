...
export default function SMTPSettings({
  ...
  const {
    isOpen: showSendEmailForm,
    open: openSendEmailForm,
    close: closeSendEmailForm,
    Model: SendEmailTestModel,
  } = useModel();
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      SMTP_FROM: Yup.string()
        ...
        .email("SMTP From must be an email address."),
      SMTP_HOST: Yup.string().required("SMTP Host is required"),
      SMTP_PORT: Yup.number("SMTP Port must be an integer.").required(
      ...
      SMTP_MAIL_ENCRIPTION: Yup.string().required(
        "SMTP Mail Encription is required",
      ),
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="mb-4">
          ...
          <input
            ...
            id="smtp_user_from"
            {...formik.getFieldProps("SMTP_FROM")}
          />
        </div>
        ...
        <div className="row">
          <div className="mb-4 col-md-4 col-sm-12">
            ...
            <Select
              options={SMTP_MAIL_ENCRIPTION_OPTIONS}
              isSearchable={false}
              defaultValue={SMTP_MAIL_ENCRIPTION_OPTIONS[0]}
              id="smtp-mail-encription"
              onChange={(option) =>
                formik.setFieldValue("SMTP_MAIL_ENCRIPTION", option.value)
              }
            />
          </div>
        </div>
        ...
        <div className="d-flex mb-5">
          <Switch
            onChange={(checked: boolean) =>
              formik.setFieldValue("SMTP_ALLOW_INSECURE_MODE", checked)
            }
            name="insecure-mode"
            checked={!!formik.values.SMTP_ALLOW_INSECURE_MODE}
            size="small"
            className="mx-2 mt-1"
          />

          <label
            htmlFor="insecure-mode"
            className="form-label"
            onClick={() =>
              formik.setFieldValue(
                "SMTP_ALLOW_INSECURE_MODE",
                !formik.values.SMTP_ALLOW_INSECURE_MODE,
              )
            }
          >
            Allow insecure mode.
