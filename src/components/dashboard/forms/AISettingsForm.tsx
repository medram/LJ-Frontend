...
export default function AISettingsForm({
  ...
  const always_active_plugins: string[] = [
    "DocumentPlugin",
    "DocumentSummarizationPlugin",
  ];
  ...
  const getInitialValues = useMemo(() => {
    ...
    settings.CHAT_AVAILABLE_PLUGINS.map((plugin: PluginType) => {
      initialValues[`CHAT_PLUGIN_${plugin.name}`] =
        always_active_plugins.includes(plugin.name) ||
        selectedPluginNames.includes(plugin.name);
    });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const selectedPlugins: SelectedPluginType[] =
        settings.CHAT_AVAILABLE_PLUGINS.filter((p: PluginType) => {
        ...
        .then((data) => {
          if (data?.errors) {
            ...
            updateAIModelSettings()
              .then((req) => {
                if (!req.data?.errors) toast.success(req.data?.message);
    ...
    <form onSubmit={formik.handleSubmit}>
      <div className="row mb-3">
        ...
        <div className="mb-6 col-md-6">
          ...
          <Select
            ...
            id="openai-model"
            defaultValue={selectedChatAgentModel}
            onChange={(option) =>
              formik.setFieldValue("CHAT_AGENT_MODEL", option.value)
            }
          />
        </div>
        <div className="mb-6 col-md-5">
          <label htmlFor="chat_agent_model_temp">
            Temperature{" "}
            <FontAwesomeIcon
              icon={faInfoCircle}
              title="Value between 0 and 1 (0 = precise | 1 = Creative)"
            />{" "}
            :
          </label>
          <input
            ...
            id="chat_agent_model_temp"
            {...formik.getFieldProps("CHAT_AGENT_MODEL_TEMP")}
          />
        </div>
      </div>
      ...
      <div className="row mb-3">
        <div className="mb-6 col-md-6">
          ...
          <Select
            options={AVAILABLE_AI_CHAT_MODELS}
            id="chat_planner_agent"
            defaultValue={selectedChatPlannerAgentModel}
            onChange={(option) =>
              formik.setFieldValue("CHAT_PLANNER_AGENT_MODEL", option.value)
            }
          />
        </div>
        <div className="mb-6 col-md-5">
          <label htmlFor="chat_planner_agent_model_temp">
            Temperature{" "}
            <FontAwesomeIcon
              icon={faInfoCircle}
              title="Value between 0 and 1 (0 = precise | 1 = Creative)"
          ...
          <input
            ...
            step={0.1}
            className="form-control"
            id="chat_planner_agent_model_temp"
            {...formik.getFieldProps("CHAT_PLANNER_AGENT_MODEL_TEMP")}
          />
        </div>
      </div>
      ...
      <div className="row mb-3">
        <div className="mb-6 col-md-6">
          <label htmlFor="openai-sum-model">
            Default Plugins Model{" "}
            <FontAwesomeIcon icon={faInfoCircle} title="Used by plugins" />:
          </label>

          <Select
            options={AVAILABLE_AI_MODELS}
            id="openai-sum-model"
            defaultValue={selectedChatSummarizationModel}
            onChange={(option) =>
              formik.setFieldValue("CHAT_TOOLS_MODEL", option.value)
            }
          />
        </div>
        <div className="mb-6 col-md-5">
          <label htmlFor="chat_TOOLS_model_temp">
            Temperature{" "}
            <FontAwesomeIcon
              icon={faInfoCircle}
              title="Value between 0 and 1 (0 = precise | 1 = Creative)"
          ...
          <input
            ...
            step={0.1}
            className="form-control"
            id="chat_TOOLS_model_temp"
            {...formik.getFieldProps("CHAT_TOOLS_MODEL_TEMP")}
          />
        </div>
      </div>
