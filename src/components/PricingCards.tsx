...
export default function PricingCards({ yearly }: { yearly: boolean }) {
  ...
  return (
    <MotionDiv className="pricing-section" key={plansToRender}>
      {!plansToRender?.length && <span>No available plans found!</span>}
      {plansToRender?.map((plan, i) => {
        return (
          <MotionItem
            className={
              plan.is_popular ? "pricing-card popular" : "pricing-card"
            }
            key={plan.id}
            variants={{
              hidden: {
          ...
          >
            ...
            <div className="pricing">
              ...
              {plan.price}
              <span className="small-text">/{yearly ? "year" : "month"}</span>
            </div>
            ...
            <div className="pricing-body">
              ...
              <ul>
                <li>
                  ...
                  <b>{plan.pdfs ? plan.pdfs : "Unlimited"}</b> PDFs
                </li>
                <li>
                  <TablerIcon icon={IconCircleCheck} className="text-success" />{" "}
                  Max PDF size:{" "}
                  <b>
                    {plan.pdf_size ? plan.pdf_size + "MB/pdf" : "Unlimited"}
                  </b>
                </li>
                <li>
                  <TablerIcon icon={IconCircleCheck} className="text-success" />{" "}
                  <b>{plan.questions ? plan.questions : "Unlimited"}</b> PDF
                  Questions
                </li>
                {plan.features?.split("\n")?.map((feature, i) => {
                  return (
