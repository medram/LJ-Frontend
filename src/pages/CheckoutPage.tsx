...
export default function CheckoutPage() {
  ...
  const { isLoading: isPaymentMethodsLoading, paymentMethods } =
    useAvailablePaymentMethods();
  ...
  const handlePayment = () => {
    ...
    if (plan.is_free || plan.price == 0) {
      subscribeToFreePlan(id)
        .then((data) => {
          if (data?.errors) return toast.error(data.message);
                ...
                {isPaymentMethodsLoading ? (
                  ...
                  <>
                    {!paymentMethods?.length ? (
                      <span>No available payment methods.</span>
                    ) : (
                      <span>Select your favorite payment method:</span>
                    )}
            ...
            <Card border="light" className="mb-4 rounded-4 shadow-sm">
              ...
              <Card.Body className="p-4">
                ...
                <div className="d-flex justify-content-between align-items-center mt-4">
                  <span>Tax:</span>
                  <span>+{settings?.CURRENCY_SYMBOL}0</span>
                </div>
                <hr />
                <Card.Title>
                  <div className="d-flex justify-content-between align-items-center mt-4">
