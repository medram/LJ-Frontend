...
export default function PlansPage() {
  ...
  const {
    ...
    open: editOpen,
    close: editClose,
    offCanvasProps: editPlanOffCanvasProps,
  } = useOffCanvas();
  const {
    isOpen: isTrialOpen,
    open: trialOpen,
    close: trialClose,
    offCanvasProps: trialOffCanvasProps,
  } = useOffCanvas();
    ...
    <>
      ...
      <div className="d-flex flex-column flex-md-row-reverse gap-3 mb-4">
        <button className="btn btn-primary" onClick={open}>
          <TablerIcon icon={IconPlus} stroke={1.25} /> New Plan
        </button>

        <Link to="../payment-gateways" className="btn btn-primary">
          <TablerIcon icon={IconCreditCard} stroke={1.25} /> Manage Payment
          Gateways
        </Link>

        <button className="btn btn-primary" onClick={() => trialOpen()}>
          <TablerIcon icon={IconLeaf} /> Manage Free Trial / demo
        </button>
      </div>
        ...
        <div className="col-12">
          <section className="bg-light rounded p-4">
            <h2 className="h6 mb-3">
              <b className="badge text-bg-secondary">Monthly plans:</b>
            </h2>
            ...
            <h2 className="h6 mb-3 mt-5">
              <b className="badge text-bg-secondary">Yearly plans:</b>
            </h2>
            <table className="table table-responsive">
              <thead>
                <tr>
      ...
      <CustomOffCanvas
        title="Edit Plan"
        placement="end"
        {...editPlanOffCanvasProps}
      >
        <EditPlanForm close={editClose} planId={planToEdit} />
      </CustomOffCanvas>

      <CustomOffCanvas
        title="Manage Free Trial / demo"
        placement="end"
        {...trialOffCanvasProps}
      >
