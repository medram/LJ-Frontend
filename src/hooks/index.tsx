...
export function usePersistedReducer(reducer: any, initialValues: object) {
  const [stats, dispatch] = useReducer(
    reducer,
    SyncFromLocalStorage("storedStats", initialValues),
  );
  const [, SyncToLocalStorage] = useLocalStorage(
    "storedStats",
    stats as object,
  );
...
export function useSettings() {
  const { data, ...rest } = useQuery("settings", getSettings, {
    staleTime: Infinity,
    suspense: true,
  });
...
export function useDashboardSettings() {
  ...
  if (
    data?.settings &&
    data?.settings?.CHAT_AVAILABLE_PLUGINS &&
    typeof data?.settings?.CHAT_AVAILABLE_PLUGINS === "string"
  )
    data.settings.CHAT_AVAILABLE_PLUGINS = JSON.parse(
      data?.settings?.CHAT_AVAILABLE_PLUGINS,
    );

  if (
    data?.settings &&
    data?.settings?.SELECTED_PLUGINS &&
    typeof data?.settings?.SELECTED_PLUGINS === "string"
  )
    data.settings.SELECTED_PLUGINS = JSON.parse(
      data?.settings?.SELECTED_PLUGINS,
    );
...
export function useDashboardPlans() {
  const { data, ...rest } = useQuery("admin.plans", getDashboardPlans, {
    staleTime: Infinity,
    suspense: true,
  });
...
export function useDashboardPlan(planId: number) {
  ...
  const plan: PlanType | null =
    data?.plans.find((plan: PlanType) => plan.id === planId) || null;
...
export function usePlans() {
  const { data, ...rest } = useQuery("plans", getPlans, {
    staleTime: 1000 * 60,
    suspense: true,
  }); // 1 minute
  const plans: PlanType[] = data?.plans;
  const monthlyPlans: PlanType[] = plans?.filter(
    (plan: PlanType) =>
      "billing_cycle" in plan && plan.billing_cycle === "monthly",
  );
  const yearlyPlans: PlanType[] = plans?.filter(
    (plan: PlanType) =>
      "billing_cycle" in plan && plan.billing_cycle === "yearly",
  );
...
export function usePlan(id: number) {
  ...
  const plan: PlanType | null =
    plans?.find((plan: PlanType) => plan.id == id) || null;
  return { ...rest, plan };
}
