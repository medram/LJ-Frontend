...
export function useCustomers() {
  const { data, ...rest } = useQuery("admin.customers", getCustomers, {
    staleTime: Infinity,
    suspense: true,
  });
...
export function usePages() {
  const { data, ...rest } = useQuery("admin.pages", getPages, {
    staleTime: Infinity,
    suspense: true,
  });
...
export function usePage(id: number) {
  const { data, ...rest } = useQuery(`admin.page.${id}`, () => getPage(id), {
    staleTime: Infinity,
  });
...
export function useSubscriptions() {
  const { data, ...rest } = useQuery(`admin.subscriptions`, getSubscriptions, {
    staleTime: Infinity,
    suspense: true,
  });
...
export function useDashboardAnalytics() {
  const { data, ...rest } = useQuery(`admin.analytics`, getDashboardAnalytics, {
    staleTime: 60000,
    suspense: true,
  });
...
export function useLC() {
  ...
  return {
    ...rest,
    isActive: data?.errors == false && data?.message === "LCD" ? true : false,
  };
}
